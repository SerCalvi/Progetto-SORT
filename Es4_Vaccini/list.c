...
LIST EnqueueOrderedF(LIST l, ItemType item) {
    NODE* new_node = createNode(item);
    ...
    if (isEmpty(l)) {
        ...
        LIST tmp_prev = tmp;
        while(!isEmpty(tmp)){
            if(tmp->item.quant_min < new_node->item.quant_min && i == 0 && c == 0){
                new_node->next = tmp;
                l = new_node;
                c = 1;
            }
            else if(tmp->item.quant_min < new_node->item.quant_min && i != 0 && c == 0){
                new_node->next = tmp;
...
LIST EnqueueOrderedC(LIST l, ItemType item) {
    NODE* new_node = createNode(item);
    ...
    if (isEmpty(l)) {
        ...
        LIST tmp_prev = tmp;
        while(!isEmpty(tmp)){
            if(tmp->item.pazienti < new_node->item.pazienti && i == 0 && c == 0){
                new_node->next = tmp;
                l = new_node;
                c = 1;
            }
            else if(tmp->item.pazienti < new_node->item.pazienti && i != 0 && c == 0){
                new_node->next = tmp;
                tmp_prev->next = new_node;
                c = 1;
            }
